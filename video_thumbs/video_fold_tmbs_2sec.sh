...
find_results="";
command="find $starting_folder -regextype posix-awk -iregex '.+($video_ext)'";
...
do
  ...
  add_star="/*";
  command="echo $video_file_orig | sed 's/.*[.]\($video_ext2\)$/\/\1/g' | sed 's/\///'g";
  ...
  command="ffmpeg -$resume_prior_ops -itsoffset -2 -i $video_file_orig -vcodec mjpeg -vframes 1 -an -f rawvideo $thumb_path_name && $resize_cmd && $crop_cmd";
  echo $command;
  eval $command;
